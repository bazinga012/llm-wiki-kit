...
idea. This file is the **canonical schema**, read by whichever agent you use
(Claude Code, Gemini CLI, Codex, opencode, Cursor, Antigravity, …). **You (the
agent) own the `wiki/` and `outputs/` layers and do all the bookkeeping. The
human curates `raw/` sources and asks questions.**
...
- This wiki is **agent-agnostic**: any agent's commands read and write the same
  files here. Don't add agent-specific structure; keep pages portable markdown.
