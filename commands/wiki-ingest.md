...
allowed-tools: Bash(sh:*), Bash(git:*), Read, Write, Edit, Glob, Grep, WebFetch
---
...
Use the path shown above as **WIKI**. Ingest a **raw source document** into the
wiki, following Karpathy's LLM-Wiki ingest flow.

Source: $ARGUMENTS

Steps:
1. Read `WIKI/CLAUDE.md` for the schema and conventions.
2. Acquire the source: local file → read it; URL → fetch (WebFetch); repo → read
   README/key docs. Save a durable markdown copy/clip into `WIKI/raw/` (convert
   PDFs/HTML to md; large binaries are gitignored — keep extracted text). Name it
...
5. Show a concise summary (pages + raw artifact), then commit in WIKI. No push
   unless asked.
