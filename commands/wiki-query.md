...
Use the path shown above as **WIKI**. Answer a question using the wiki,
following Karpathy's LLM-Wiki query flow.

Question: $ARGUMENTS

Steps:
1. Read `WIKI/index.md` to locate relevant pages, then read those pages (and any
   `WIKI/raw/` sources they cite if more depth is needed).
